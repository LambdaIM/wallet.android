 ...
 * tbb兑换lamb
 */
public class TbbExchangeFragment extends LazyLoadFragment<TbbExchangeView, TbbExchangePresenter> implements TbbExchangeView {
    ...
    protected int setContentView() {
        return R.layout.fragment_tbb_exchange;
    }

    @Override
    public TbbExchangePresenter initPresenter() {
        return new TbbExchangePresenter(getActivity());
    }
    ...
    protected void onFragmentFirstVisible() {
        mToken.addTextChangedListener(new TextWatcher() {
            ...
            public void afterTextChanged(Editable s) {
                try {
                    tbb = mToken.getText().toString().trim().toString();
                    lamb = StringUtils.deletzero(BigDecimalUtil.multiply(new BigDecimal(tbb), new BigDecimal(3000)).toString());
                    mGetToken.setText(lamb);
                } catch (Exception e) {
                    e.printStackTrace();
                    mGetToken.setText("0");
                    lamb = null;
    ...
    public void getGasDataHttp(GasBean gasBean) {
        ...
        PasswordDialog mPasswordDialog = new PasswordDialog(getActivity(), new com.lambda.wallet.view.dialog.passworddialog.PasswordCallback() {
            ...
            public void sure(String password) {
                ...
                msgsBean.setType(Constants.SignType.TBB2LAMMB);
